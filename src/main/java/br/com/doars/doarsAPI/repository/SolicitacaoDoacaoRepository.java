...
public interface SolicitacaoDoacaoRepository extends JpaRepository<SolicitacaoDoacao, Long> {
    ...
    @Query(value = "select distinct s.* from solicitacao_doacao s, entidade e, solicitacao_doacao_tipos_sanguineos sdt, tipo_sanguineo tp \n" +
            "where s.entidade_id = e.id and s.id = sdt.solicitacao_doacao_id and sdt.tipos_sanguineos_id = tp.id and s.descricao like %?1% and s.entidade_id = ?2 and s.ativo = true", nativeQuery = true)
    Page<SolicitacaoDoacao> findAllByEntidadeIdAndDescricao(Pageable pageable, String search, Long id);

    @Query(value = "select distinct s.* from solicitacao_doacao s, entidade e, solicitacao_doacao_tipos_sanguineos sdt, tipo_sanguineo tp \n" +
            "where s.entidade_id = e.id and s.id = sdt.solicitacao_doacao_id and sdt.tipos_sanguineos_id = tp.id and tp.id in ?1 and s.descricao like %?2% and s.entidade_id = ?3 and s.ativo = true", nativeQuery = true)
